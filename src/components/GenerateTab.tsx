...
export function GenerateTab({ onPortfolioGenerated, portfolio }: Props) {
    ...
    useEffect(() => {
        const executeGeneration = async (capital: number, riskEnum: string) => {
        ...
        if (window.pendingAiAction === 'generate_portfolio') {
            const args = window.pendingAiActionArgs || {};
            window.pendingAiAction = undefined;
            window.pendingAiActionArgs = undefined;
            const capital = args.capital || amount;
            setTimeout(() => executeGeneration(capital, args.risk), 300);
        }
        ...
        const handleAction = async (e: any) => {
            ...
            if (action.name === 'generate_portfolio') {
                ...
                executeGeneration(capital, action.arguments.risk);
            }
        };
